...
import { useFMS, DEFAULT_STATE, getSeededStateForUser } from '../context/FMSContext';
...
const Auth: React.FC<AuthProps> = ({ mode: initialMode, onNavigate }) => {
  ...
  const handleImmediateDemoLogin = (roleToUse: 'Admin' | 'User') => {
    setIsLoading(true);
    setError('');
    setSuccess('');
    ...
    const activeUser = registeredUsers.find((u: any) => u.email.toLowerCase() === targetEmail);
    if (activeUser && (activeUser.status === 'Banned' || activeUser.isBanned)) {
      setError(language === 'id' ? 'Akun Anda telah ditangguhkan (Banned) oleh Administrator.' : 'Your account has been suspended (Banned) by the Administrator.');
      setIsLoading(false);
      return;
    }
    ...
    setTimeout(() => {
      ...
      const uniqueSeed = getSeededStateForUser(targetEmail, roleToUse);
      const targetStateData = {
        ...uniqueSeed,
        role: roleToUse,
        currentUserEmail: targetEmail,
        users: [
          { id: 'U1', name: targetName, email: targetEmail, role: roleToUse, subscription: 'Pro Plan' }
        ]
      };

      // Direct write to user storage & active user email key
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    // Load registered users from custom repository
    const registeredUsers = getRegisteredUsers();
    ...
    if (mode === 'register') {
      ...
      // Add a fresh, newly registered account
      const newUser = {
        name: name.trim(),
        phone: phone.trim(),
        email: normalEmail,
      ...
      localStorage.setItem('fms_registered_users', JSON.stringify(updatedUsers));

      // Create pristine state profile (empty data pools for a true fresh start)
      const freshNewState = {
        ...DEFAULT_STATE,
        currentUserEmail: normalEmail,
        role: normalEmail === 'mochamadraflyrasyidin@gmail.com' ? 'Admin' : 'User',
        subscription: 'Free' as const,
        users: [
          { id: 'U1', name: name.trim(), email: normalEmail, role: normalEmail === 'mochamadraflyrasyidin@gmail.com' ? 'Admin' : 'User', subscription: 'Free' }
        ],
        coa: [],
        entities: [],
        transactions: [],
        ...
        projects: [],
        vendors: [],
        payrollRuns: [] // Clean, empty, customizable environment with 0 metrics
      };
      ...
      setTimeout(() => {
        setIsLoading(false);
        // Login directly with this new credentials
        dispatch({
          type: 'LOGIN_USER',
          payload: { email: normalEmail, stateData: freshNewState }
        });
        setSuccess('Registration successful! Launching setup...');
        setTimeout(() => {
      ...
      // Enforce registered check -> "pastikan juga ketika login emailnya itu harus terdaftar, jangan malah email belum terdaftar tetapi bisa melakukan login, perbaiki!"
      if (!activeUser) {
      ...
      // Authentic user success! Now load state
      setTimeout(() => {
        setIsLoading(false);
        
        let targetStateData: any;

        if (activeUser.isDemo || normalEmail === 'demo@fms.com' || normalEmail === 'demo_admin@fms.com' || normalEmail === 'demo_user@fms.com') {
          const userRole = activeUser.demoRole || (normalEmail === 'demo_user@fms.com' ? 'User' : 'Admin');
          
          const storedState = localStorage.getItem(`fms_state_user_${normalEmail}`);
          if (storedState) {
            targetStateData = JSON.parse(storedState);
            targetStateData.currentUserEmail = normalEmail;
            targetStateData.role = userRole;
            // Ensure demo users are present
            if (!targetStateData.users || targetStateData.users.length < 2) {
              targetStateData.users = [ ...DEFAULT_STATE.users ];
            }
          } else {
            // Demo Account -> AUTO SEEDS distinct, custom, simulated dummy financials
            const uniqueSeed = getSeededStateForUser(normalEmail, userRole);
            targetStateData = {
              ...uniqueSeed,
              currentUserEmail: normalEmail,
              role: userRole,
          ...
          // Regular registered account -> Pull from storage or create new if not found
          const storedState = localStorage.getItem(`fms_state_user_${normalEmail}`);
          if (storedState) {
            targetStateData = JSON.parse(storedState);
            // Dynamic role override to enforce strict roles
            targetStateData.role = normalEmail === 'mochamadraflyrasyidin@gmail.com' ? 'Admin' : 'User';
            if (targetStateData.users && targetStateData.users.length > 0) {
              targetStateData.users[0].role = targetStateData.role;
            }
          } else {
            // Settle default clean state
            targetStateData = {
              ...DEFAULT_STATE,
              currentUserEmail: normalEmail,
              role: normalEmail === 'mochamadraflyrasyidin@gmail.com' ? 'Admin' : 'User',
              subscription: 'Free' as const,
              users: [
                { id: 'U1', name: activeUser.name, email: normalEmail, role: normalEmail === 'mochamadraflyrasyidin@gmail.com' ? 'Admin' : 'User', subscription: 'Free' }
              ],
              coa: [],
              entities: [],
              transactions: [],
