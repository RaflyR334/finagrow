...
import { useFMS, getSeededStateForUser } from '../context/FMSContext';
...
const Settings: React.FC = () => {
  ...
  // 2. Data Restore: Restores user-specific original seeded states
  const handleRestoreData = () => {
    const emailKey = state.currentUserEmail || 'demo_admin@fms.com';
    const roleKey = state.role || 'Admin';
    const seededState = getSeededStateForUser(emailKey, roleKey);
    
    // We retain profile identity
    const restoredState: FMSState = {
      ...seededState,
      currentUserEmail: emailKey,
      role: roleKey,
      users: state.users && state.users.length > 0 ? state.users : seededState.users
    };
    ...
    triggerToast(
      language === 'id' 
        ? 'Data berhasil dipulihkan ke kondisi seed bawaan!' 
        : 'Data successfully restored to the default seeded state!'
    );
  };
